...
- Team pipeline: raw csvs + `player_game_stats.xlsx`/`player_master_table.xlsx` (for the real-PIR `funnel_actual_pir_*` KPIs) -> `src/build_team_kpis.py` -> `src/build_team_master_table.py`; outputs in `data/curated/`. The player master reads no team workbook (its 20 canonical team names come from the price list), so the dependency is one-way: rebuild order is game stats -> player master -> team KPIs -> team master.
