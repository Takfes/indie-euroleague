...
- **Column layout.** The order, the dropped duplicates and the excluded columns live in
  ...
  Kaggle distribution KPIs (the 11 contribution families last: `_avg`, `_sd`, `_cv`, `_p10`, `_p50`, `_p90`,
  `_range` each). Within a stat family the sub-order is one pattern: per-game attempts, season-total
  ...
  - Duplicates (`DUPLICATE_COLUMNS`). A stat two sources both report appears once, decided on value agreement
    ...
    `offensive_rating_lineup` in total and offense) are kept per view anyway. The layout lists place the
    kept columns only, so changing an entry needs a layout edit too: to keep a dropped column as well,
    remove its entry and place it; to flip a pair, swap the entry, place the newly kept column and remove
    the newly dropped one (the build raises "In the Master but not in the layout" / "In the layout but not
    in the Master" otherwise).
